        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div>
            ...
            <div class="flex justify-center py-3 bg-[#1d242e] items-center">
              <img
                :src="`${baseURL}/litbang/thumbnail/${data?.thumbnailPath}`"
                class="max-w-96"
                alt=""
              />
            </div>
...
import {
  updatePenelitianSchema,
  type TUpdatePenelitianSchema,
} from "~/schema/litbang/update";
...
const { defineField, errors, handleSubmit } = useForm<TUpdatePenelitianSchema>({
  validationSchema: updatePenelitianSchema,
});
...
const thumbnailDefaultUrl = ref("");
const videoDefaultUrl = ref("");
  ...
  `/api/litbang/${id}`,
  {
    ...
    onResponse: ({ response }) => {
      if (response._data) {
        ...
        thumbnailDefaultUrl.value = `${baseURL}/litbang/thumbnail/${response._data.thumbnailPath}`;
        videoDefaultUrl.value = `${baseURL}/litbang/video/${response._data.videoPath}`;
      }
    },
  }
);
...
async function handleGantiThumbnail() {
  ...
  const postRequest = await axios.postForm(
    `/api/litbang/ganti-thumbnail/${id}`,
    payload
  );
...
const onSubmit = handleSubmit(async (payload: TUpdatePenelitianSchema) => {
  loading.value = true;

  const createRequest = await axios.patch(`/api/litbang/${id}`, payload);
